% Make a video
function []=VideoWriting_Growth(Lseg,N,Rt,tdata,R_all,Lseg_all,...
...
XLIM = [0-Lseg_all(1,1) sum(Lseg_all(1,:))*1.25];
...
for i=1:n_time_video
    ...
    Lseg = Lseg_all(video_time_idx(i),:);
    plot_notochord_video(Lseg,N,R,Rt,tdata(video_time_idx(i)),P,Pi,PE,[tdata(1:video_time_idx(i))*tr;PiE_all(1:video_time_idx(i),1)'],J_loss/3,tr,CLIM,CbOri,XLIM,FontSetup);
    drawnow;
    hold off;
